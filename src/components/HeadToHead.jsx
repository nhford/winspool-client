            ...
            <tr>
              {labels.map((label, i) => (
                <th
                  ...
                  onClick={() =>
                    i == 0
                      ? handleSort(headers[i], sortingUtil)
                      : recordSort(headers[i], sortingUtil)
                  }
                >
                  {label}
                </th>
            ...
            {data.map((row, rowIndex) => (
              <React.Fragment key={row.owner}>
                <tr
                  ...
                  onClick={() => toggleExpand(row.owner)}
                >
                  {headers.map((col, i) => (
                    <td
                      key={i}
                    ...
                    >
                      ...
                      >
                        <thead>
                          <tr>
                            <th
                              onClick={() => handleSort("abbrev", sortingTeams)}
                            >
                              Team
                            </th>
                            {labels.map(
                              (label, i) =>
                                i > 0 && (
                                  <th
                                    key={i}
                                    onClick={() =>
                                      recordSort(headers[i], sortingTeams)
                                    }
                                  >
                                    {label}
                                  </th>
